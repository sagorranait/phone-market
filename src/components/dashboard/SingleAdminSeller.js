...
function SingleAdminSeller({seller, numbers, verifiedHandler, deleteSellerHandler, buyer}) {
  ...
  return (
   <ItemTemplate>
      ...
      <div className='product-seller'>
         <p>{displayName} {buyer ? '' : <span>{verified && <img src={Verified} alt="Verified" />}</span>}</p>
         <p>Call: {number}</p>
         <p>Area: <span>{location}</span></p>
      </div>
      <div className={`product-action adminUser-action ${buyer && 'buyer-btn'}`}>
         {buyer ? '' : 
            <>
